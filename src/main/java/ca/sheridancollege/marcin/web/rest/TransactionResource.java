...
import ca.sheridancollege.marcin.repository.TransactionRepository;
...
/**
 * REST controller for managing {@link ca.sheridancollege.marcin.domain.Transaction}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Transaction> result = transactionRepository
            ...
            .map(existingTransaction -> {
                if (transaction.getSubTotal() != null) {
                    existingTransaction.setSubTotal(transaction.getSubTotal());
                }
                if (transaction.getTax() != null) {
                    existingTransaction.setTax(transaction.getTax());
                }
                if (transaction.getTotal() != null) {
                    existingTransaction.setTotal(transaction.getTotal());
                }
                if (transaction.getLastFourDigits() != null) {
                    existingTransaction.setLastFourDigits(transaction.getLastFourDigits());
                }
